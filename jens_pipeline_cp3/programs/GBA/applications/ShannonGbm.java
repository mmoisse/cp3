...
** Author: Xuehui Li
** Date: Dec, 2005
** "gbm" or "GBM" is the abbrevaition of "A graph-based method for detecting low-complexity reions".
** This is program is used to find low-complexity regions in sequences when Shannnon Entropy is used to repleace our new complexity measures in GBA.
** NOTE: all vertices and edges have topological orders
** There are totally six input paramaters. The first one is the sequence file 
...
** One example (to generate LCR blocks instead of masked sequences) with fixed sampling, i.e., 27 LCRs from five sequences masked by SEG:
** java applications.ShannonGbm wShannonEntropy/input/seqFromFlybase swissprotLearnedMatrices/wForgetRate/normalized/combinedMatricesRowByRow095 3 15 5 knowledge/lcrs 0 > wShannonEntropy/LCRBlocks/mimMaskedBlocks
** if conditional-sampling (i.e., G_t / sqrt(t) <= C * u_t)is used, the "knowledge/lcrs" will not make sense any more. 
*/
...
class ShannonGbm {
  ...
  // vertices and edges are vectors used to keep all the vertices, edges in a graph generated from a sequence, respectively. vertexQueue is a vector used to keep all vertices whose indegree is zero. lps is a vector used to keep all the longest paths in all connected subgraphs of a sequence. Every longest path in lps is a vector of vertices, excluding the dummy source.
  // subVertices and subEdges are vectors used to keep all the vertices, edges in a connected-graph which is a subgraph of the graph generated from a sequence, respectively.   
 
  private static final double comCut = 3.7491631225622157; //the complexity cutOff value for extending the longest-path intervals.
  ...
  public ShannonGbm ( String fileName ) {
    // the graph is given in a file where every line represents an edge and has the fromat of "source sink weight"
    initializeAlphabet();
    try{
      f = new File ( fileName );  
  ...
  public void readRNRMatrices( String matricesFile ) {
	...
	for ( int j = 0; j < 20; j++ ) {
	  ...
	  if ( index != -1 ) {
	    nonRepeatMatrix[i][j] = Float.parseFloat( row.substring( 0, index ));
	  }	
	  else {
      ...
      rfm.readLine(); rfm.readLine(); rfm.readLine();
      for ( int i = 0; i < 20; i++ ) {
	row = rfm.readLine();
	row = rfm.readLine();
	row = row.trim();
	for ( int j = 0; j < 20; j++ ) {
	  int index = row.indexOf( "   " );
	  if ( index != -1 ) {
	    repeatMatrix[i][j] = Float.parseFloat( row.substring( 0, index ));
	  }	
	  else {
 ...
 public void printMatricesRowByRow() {
  ...
  /*  
  public void getComCut( String fileName ) {
    ...
    try { 
      ...
      double com, sum = 0, num = 0;
      line = rf.readLine();
      while ( line != null ) { 
	line = line.trim();
	com = cc.calculate2LetterEntropyWScoMatrix( line );
	//com = cc.calculateModifiedEntropy( line );
	sum = sum + com;
	num = num + 1;
	line = rf.readLine();
      }
      comCut = sum / num;
       System.out.println( "The cut-off value is: " + sum + " / " + num + " = " + comCut );
      rf.close();
    }
    catch( IOException ex ){
    }  
...
*/
  ...
  public void findLetterPercentageVer( float lF ) {
    ...
    float per = len / lF;
    // System.out.println( "The letter percentage after vertex construction is: " + len + " / " +  lF + " = " + per  );
  ...
  public void computePercentage( float l, Vector al ) {
    int len = al.size();
    float per = len / l;
    System.out.println( "The percentage is: " + len + " / " + l + " = " + per  );
    for ( int i = 0; i < len; i ++ ) {
 ...
 // return the actual weight of the edge ???  
  public boolean checkConditions ( int i, int j, int p, int q, int th1, int th2, int th3 ) {
  ...
  public void printVertexQueue() {
    ...
    for ( int i = 0; i < l; i++ ) {
      ...
      if ( previous != null)
      System.out.println( v.getStartLetter() + "A" + v.getEndLetter() + "  " + v.getIndegree() + " " + v.getWeight() + " " + v.getLP() + " " + previous.getStartLetter() + "A" + previous.getEndLetter());
      else 
	System.out.println( v.getStartLetter() + "A" + v.getEndLetter() + "  " + v.getIndegree() + " " + v.getWeight() + " " + v.getLP()); 
    }
  }
...
// m = 0 for subgraph, m = 1 for the whole sequence
  public void printEdges( int m ) {
    ...
    for ( int i = 0; i < length; i++ ) {
      ...
      sinkVer = e.getSink();
      System.out.println( sourceVer.getStartLetter() + "A" + sourceVer.getEndLetter() + "   lp: " + sourceVer.getLP()+ "   indegree: "+  sourceVer.getIndegree() +  "     "+ sinkVer.getStartLetter() + "A" + sinkVer.getEndLetter() + "   lp: " + sinkVer.getLP() + "   indegree: " + sinkVer.getIndegree() + "    weight: "+ e.getWeight() );
    }
  }
  ...
  public void printLongestPath( Vector lp ) {
    ...
    for ( i = 0; i < length; i++ ) {
      v = (Vertex) lp.get( i );
      System.out.print( v.getStartLetter() + "A" + v.getEndLetter() + "  ");
    }
    System.out.println();
  }
  ...
  public void extractConnectedGraph() {
	...
	int m = 0;
	int len = edges.size();
	while (( same ) &&( m < len )) { // remove all those edges having the same source vertex as the first edge 
	  ...
	  if ( v.equals( ver )) {
	    ...
	    ver = e.getSink();
	    subVertices.add( ver );
	    tmpQueue.add( ver ); // put ver ( the sink of the edge ) into tmpQueue;
	...
	boolean found = false;
	int len = edges.size();
 //find the starting positon of those edges who start at the first vertex from tmpQueue	
	while (( !found ) && ( m < len )) { // skip all edges starting with the vertex from the tmpQueue
	  e = (Edge)edges.elementAt( m );
	  ...
	  if ( v.equals( sr ))
	    ...
	    if ( v.equals( si )) {
	      ...
	      if ( !exist ) { 
		subVertices.add( sr );
		tmpQueue.add( sr ); // work on edges whose sink vertex is the same as the vertex from tempQueue
	...
	boolean same = true;
	while ( same ) { // remove all those edges starting with the vertex from tmpQueue to subEdges 
	  len = edges.size();
	  if ( m < len ) {
	    e = (Edge)edges.elementAt( m );
	    Vertex ver = e.getSource();
	    if ( v.equals( ver )) {
	      ...
	      subEdges.add( e ); 
	      ver = e.getSink();
	      boolean exist = checkExistence( ver ); // check whether the sinkVertex is in the subVertices or not
	      if ( !exist ) {
		subVertices.add( ver );
		tmpQueue.add( ver ); // put ver ( the sink of the edge ) into tmpQueue;
		if ( !(vertices.remove( ver )))
  ...
  public int workOnSequence( String str, int th1, int th2, int th3, int th4 ) {
    ...
    while ( !(edges.isEmpty())) {
      ...
      Vector lp = identifyLCRs(); // the longest path in a connected subgraph 
  ...
  // combine all letters from a sequence on different lines( stings) into a single line ( string )
  ...
  public Vector getPositions (  int k ) {
    ...
    Vertex v = new Vertex();
    // longest path
    for ( int i = 0; i < k; i++ ) { // get positions from those vertices in lps
      lp = (Vector)lps.get( i );
      int len = lp.size();
      for ( int j = 0; j < len; j++ ) {
	v = (Vertex)lp.get( j );
  ...
  public Vector sortPositions( Vector pos) {
    ...
    for ( int i = 0; i < len; i++ ) {
      ...
      //generate blocks of continuous positions. Say, the sorted integer array is 3,4,5,6 8,9,10,11,12,13,29,30,31. It can be represented as a vector of three strings( blocks ):  3-6, 8-13, 29-31.
  ...
  public Vector extend ( int startPos, int endPos, int limit, String direction, String seq ) {
	...
	com1 = cc.calculateEntropy(extReg );
	extReg = seq.substring( pointer, endPos );
	com2 =  cc.calculateEntropy(extReg );
	if ( com1 > com2 ) {
	  if ( !dec ) {
	    dec = true;
	  ...
	  if ( dec ) {
	    ...
	    if ( com1 < comCut ) {
	      ...
	      //System.out.println( "decRegs added 11111111 " + startDecPos + "-" + endDecPos );
	    }
	  }
	-- pointer;
      }
	...
	while ( ( pointer > limit ) && ( dec ) ) {
	  ...
	  if ( com1 < com2 ) {
	    ...
	    if ( com1 < comCut ) {
	      decRegs.add( 0, ( startDecPos ) + "-" + ( endDecPos ));
	      //System.out.println( "decRegs added 222222222222 " + startDecPos + "-" + endDecPos );
	    }
	  }
	  -- pointer;
	}
      }
// the left extension touches the end of the last block of the current lcr blocks 
      if (( pointer == limit ) && ( dec ) ) { 
	startDecPos = pointer + 2;
	if ( com2 < comCut ) {
	  decRegs.add( 0, ( startDecPos ) + "-" + ( endDecPos ));
	  //System.out.println( "decRegs added 333333333333 " + startDecPos + "-" + endDecPos );
	} 
      }
      if ( decRegs.size() == 0 ) {
	...
	if ( com1 > com2 ) {
	  if ( !dec ) {
	    dec = true;
	    //System.out.println( "from false to true33333333333333333" );
	  ...
	  if ( dec ) {
	    ...
	    if ( com1 < comCut ) {
	      ...
	      //System.out.println( "decRegs added 4444444 " + startDecPos + "-" + endDecPos );
	    }
	  }
	++ pointer;
      }
      if ( ( dec ) && ( pointer == ( endPos + 15 )) ) { 
	// keep extending until the complexity starts increasing, which means that several blocks generated from the longest path can be included into lcrs during one call of the 'extend()' based on a block 
	while (( dec ) && ( pointer < limit  )) { 
	  ...
	  if ( com1 < com2 ) {
	    ...
	    if ( com1 < comCut ) {
	      decRegs.add(( startDecPos ) + "-" + endDecPos );
	      //System.out.println( "decRegs added 5555555555555555555 " + startDecPos + "-" + endDecPos );
	    }
	  }
	  ++ pointer;
	}
	if (( pointer == limit ) && ( dec )) { 
	  endDecPos = limit - 1;
	  //System.out.println( "decRegs added 66666666666 " + startDecPos + "-" + endDecPos );
	  decRegs.add( 0, ( startDecPos ) + "-" + ( endDecPos ));
	}
      }
      if ( decRegs.size() == 0 ) {
  ...
  public boolean checkContribution( String currentBlock, Vector decRegs, String seq ) {
    ...
    while ( ( i < len ) && ( !contributed )) {
      ...
      //System.out.println( "block: " + block+ "   currentBlock: "+ currentBlock );
   ...
   public Vector pickUpDrop ( Vector blocks, String seq ) {
     ...
     while (( !tmpBLOCKS.isEmpty() )) {
       ...
       boolean extendToLeft = true; // whether to extend towards the left 
       boolean find = false; // find the current extending block    
       // currentBlock can start in the middle of a block, or has the same starting position as a block and it doesn't have to be the block after the previous currentBlock
	 ...
	 while (( !find ) && ( !(tmpBLOCKS.isEmpty()) ))  { // get the current block
	   ...
	   //System.out.println( "find currentBlock: "+  startPos + " " + endPos);
	 ...
	 double com = 0;
	 index = currentBlock.indexOf( "-" );
	 int cbStart = Integer.parseInt( currentBlock.substring( 0, index ))- 1;
	 int cbEnd = Integer.parseInt( currentBlock.substring( index + 1 )) ;
	 com =  cc.calculateEntropy(seq.substring( cbStart, cbEnd ));
	 boolean contributed = false;
	 if ( frontLcrs.size() != 0 ) {
	   // get the start position of the first block in frontLcrs as the start position of the block to be added into lcrs
	   tmpBlock = (String)frontLcrs.elementAt( 0 );
	   index = tmpBlock.indexOf( "-" );
	   lcrBlockStart = Integer.parseInt( tmpBlock.substring( 0, index ) );
	   if ( com > comCut ) { 
	     // check whether the current block contributes to the complexity-decreasing regions or not
	     contributed =  checkContribution(  seq.substring( cbStart, cbEnd ), frontLcrs, seq );
	     if ( !contributed ) 
	 ...
	 boolean combine = false; // whether to combine the last block in lcrs from frontLcrs  and the block to be added into lcrs from backLcrs
	 if (( !contributed ) && ( com > comCut )){
	   contributed = checkContribution( seq.substring( cbStart, cbEnd ), backLcrs, seq );
	   if ( !contributed ) {
	   ...
	   if ( frontLcrs.size() != 0 )
	     combine = true; 
	 // get the end position of the last block in backLcrs as the end position of the block to be added into lcrs
	 if ( !( backLcrs.isEmpty())) {
	   ...
	   if ( combine ) {
	     ...
	     if ( com < comCut ) {
	       ...
	       //System.out.println( "contributed to the back, com > comCut " );
	       lcrBlockStart = startPos;
	     }
	     else {
	   ...
	   if((frontLcrs.size() == 0 ) && ( !contributed ) && ( com < comCut )) { 
	     lcrs.add( currentBlock );
	   }
	 }
	 // check whether to combine the last two blocks in the current lcrs
	 //len = lcrs.length();
	 /*
	 System.out.print( "current lcrs:  " );
	 printLCRs( lcrs );
	 */
       }   
     }
   ...
   public Vector mergePurge( Vector lcrs ) {
	 ...
	 int end = Integer.parseInt( currentBlock.substring( endIndex +1 ));
	 nextBlock = (String) tmpLcrs.elementAt( i + 1 );
	 int startIndex = nextBlock.indexOf( "-" );
	 int start = Integer.parseInt( nextBlock.substring( 0, startIndex )); 
	 if (( end == ( start - 1 )) || ( end == start )) {
	   //System.out.println( currentBlock + " " +  nextBlock );
	   currentBlock = currentBlock.substring( 0, endIndex ) + "-" + nextBlock.substring( startIndex + 1 );
	   tmpLcrs.remove( i );
	   tmpLcrs.remove( i );
    ...
    while ( i < len ) {
      currentBlock = (String) tmpLcrs.elementAt( i );
      int index = currentBlock.indexOf( "-" );
      int start = Integer.parseInt( currentBlock.substring( 0, index ));
      int end = Integer.parseInt( currentBlock.substring( index + 1 ));
      if (( end - start ) < 7 )
	tmpLcrs.remove( i );
      else 
	++i;
  ...
  public boolean checkCombinedSubBlock( String seq1, String seq2, double cCut ) {
    boolean delete = true;
    double com =   cc.calculateEntropy( seq1 + seq2 );
    if ( com > cCut )
      delete = false;
    return delete;
  }
  ...
  public String findAlignment( String seq1, String seq2 ) {
    ...
    try {
      ...
      if ( similarLen > 4 ) { // only if the length of similar and same letters is greater than 4       
  ...
  public Vector checkLeftRegs( int aliStart, int aliEnd, int start, int end, String seq, double cCut ) {
    Vector left = new Vector();
    double com = 0;
    if ( aliStart > 7 )  { // the length of the left region must be longer than 7
      com =  cc.calculateEntropy( seq.substring( start - 1, start + aliStart - 2 ));
      //System.out.println( "left1: " + seq.substring( start - 1, start + aliStart - 2 ) + "  " + com + "  " +  start + "-" + ( start + aliStart - 2 ));
      if ( com <= cCut ) 
	left.add( 0, start + "-" + ( start + aliStart - 2 ));
    }
    if ( ( end - start + 1 - aliEnd ) > 7 ) {
      com =  cc.calculateEntropy(seq.substring( start + aliEnd - 1, end ));
      //System.out.println( "left2:" + seq.substring( start + aliEnd - 1, end ) + "  " + com + "  " + ( start + aliEnd ) + "-" + end );
      if ( com <= cCut )
  ...
  public Vector addToResult( Vector result, Vector left ) {
	...
	if ( j < result.size() ) {
	  ...
	  index = str2.indexOf( "-" );
	  int startResult = Integer.parseInt( str2.substring( 0, index ));
	  if ( endLeft < startResult ) {
	    found = true;
	    // System.out.println( "Insert left into result: " + str1 + "  " + str2 );
	    result.add( j , str1 );
	    j = j + 2;
	  }
	  else 
  ...
  public Vector checkAdjBlock ( int start1, int end1, String adjBlock, String seq, double cCut, String mark ) {
    Vector result = new Vector();
    double com = 0;
    String seq1 = seq.substring( start1 - 1, end1 );
    //System.out.println( "current block: " +start1 + " " + end1 + "  " + seq1 );
	...
	index1 = aliPos.indexOf( "-" );
      ...
      int aliStart2 = Integer.parseInt ( aliPos.substring( 0, index1 ));
      int aliEnd2 = Integer.parseInt ( aliPos.substring( index1 + 1, index2 ));
      String aliSeq1 = new String(), aliSeq2 = new String();
      if ( mark.equals( "front" )) {
	aliSeq2 = seq.substring( start2 + aliStart2 - 2, start2 + aliEnd2 - 1 );
	//System.out.println( "the first aligned subSeq: " + aliStart2 + "  " + aliEnd2 + " " + aliSeq2 ); 
      }
      else {
	aliSeq1 = seq.substring( start1 + aliStart2 - 2, start1 + aliEnd2 - 1 );	
	//System.out.println( "the first aligned subSeq: " + aliStart2 + "  " + aliEnd2 + " " + aliSeq1 ); 
      }    
     ...
     if ( mark.equals( "front" )) {
       aliSeq1 = seq.substring( start1 + aliStart1 - 2, start1 + aliEnd1 - 1 );	
       //System.out.println( "the second subSeq: " + aliStart1 + "  " + aliEnd1 + " " + aliSeq1 ); 
     }
     else {
       aliSeq2 = seq.substring( start2 + aliStart1 - 2, start2 + aliEnd1 - 1 );	
       //System.out.println( "the second subSeq: " + aliStart1 + "  " + aliEnd1 + " " + aliSeq2 ); 
     }
     ...
     if ( mark.equals( "front" )) {
       ...
       if ( decOrNot ) {      	
	 result.add( ( start1 + aliStart1 - 1 )+ "-" + ( start1 + aliEnd1 - 1 ) );
	 //System.out.println( "added to result1: " + ( start1 + aliStart1 - 1 )+ "-" + ( start1 + aliEnd1 - 1 ));
	 Vector left = checkLeftRegs( aliStart1, aliEnd1, start1, end1, seq, cCut );	
	 result = addToResult( result, left );
       ...
       decOrNot = true;
       if ( decOrNot ) {
	 result.add( ( start1 + aliStart2 - 1) + "-" + ( start1 + aliEnd2 - 1) );
	 //System.out.println( "added to redult2: " + ( start1 + aliStart2 - 1) + "-" + ( start1 + aliEnd2 - 1));
	 Vector left = checkLeftRegs( aliStart2, aliEnd2, start1, end1, seq, cCut );
	 result = addToResult( result, left );
	 result.add( ( start2 + aliStart1 - 1 ) + "-" + ( start2 + aliEnd1 - 1 ) );
	 //System.out.println( "added to redult3: " + ( start2 + aliStart1 - 1 ) + "-" + ( start2 + aliEnd1 - 1 )); 
  ...
  public Vector checkDeletability ( Vector lcrs, int maxIndex, String seq, double cCut ) {
	...
	result = checkAdjBlock( start1, end1, block, seq, cCut, "back" );
	if ( result.size() != 0 ) 
	  result.add( "back" );
      }
    }
  ...
  public Vector filter ( Vector lcrs, String seq ) {
	...
	int index = str.indexOf( "-" );
	str = seq.substring( Integer.parseInt( str.substring( 0, index )) - 1, Integer.parseInt( str.substring( index + 1 ) ));
	singleCom = cc.calculateEntropy( str );
	...
	int index = str.indexOf( "-" );
	//System.out.println( "current block?" + str.substring( 0, index ) + "?" + str.substring( index + 1 ) );
	str = seq.substring( Integer.parseInt( str.substring( 0, index )) - 1, Integer.parseInt( str.substring( index + 1 ) ));
	...
	if ( singleCom >= cCut ) {
	  ...
	  if ( rSize != 0 ) {
	    ...
	    // System.out.println( "remove: "+ (String)tmpLcrs.remove( j ));
	    tmpLcrs.remove( j );
	    for ( int k = 0; k < rSize; k++ ) {  // add 'result' Vector into tmpLcrs in order
	    ...
	    //System.out.println();
	    // if ( j < tmpLcrs.size())
	    //System.out.println( "the next one:" + tmpLcrs.elementAt( j ));
	    if ( fromBack ) {
	      //System.out.println( "yes, from back " );
	      //System.out.println( "removed: "+ tmpLcrs.remove( j ));
	      tmpLcrs.remove( j );
	    }
	    len = tmpLcrs.size();
	  }
  ...
  public void startt( int th1, int th2, int th3, int th4, String mark ) {
	...
	if ( str != null ) {    
	  ...
	  Vector lcrs = postProcess( i, str, mark );//process all longest paths from every connected subgraph
	  
	  /*
	  computeLCRPercentage( lcrs, str );
	  printLCRs( lcrs, str );
	  */
	}
      }
  ...
  public static void main ( String args[] ) {
    ...
    int th4 = th2;
    ShannonGbm g = new ShannonGbm( args[0]);  
    ...
    /*    
     try {
      Sequence s1 = SequenceParser.parse("VVVVVV" );
      Sequence s2 = SequenceParser.parse( "LAELLAKKSDRDSPKK");
      Alignment alignment = SmithWatermanGotoh.align(s1, s2, MatrixLoader.load("BLOSUM62"), 10f, 0.5f);
System.out.println( "similarity: " + alignment.getSimilarity() );
      String result  = new Pair().format(alignment);
      System.out.println( "*****************" );

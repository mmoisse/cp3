...
<?php
	...
	//Read the two variables from the stat file, update them and rewritten them
	...
	else
	{
		...
		fwrite($resfp, '<body>'."\n");
		*/
		// Scanning the destfile we shall count the number of '<' in it
		//and shall count the number of query in it.
		//At least let's try it
		//read the content of $desFile in a string
		if( false == ($totalseq=file_get_contents($destFile)))
			echo "Could not read file.";
		//Now search the substring
		$count = substr_count($totalseq, ">");
		//echo 'Count: '.$count;
		if(true == file_exists('statFile.txt'))
		{
			@$statfp = fopen('statFile.txt', 'r+');
			//Handling error message
			if(!$statfp)
			{
				echo '<p><strong> Your request cann\'t be processed at this time.<br />
			...
			if(false == flock($statfp, LOCK_EX))
			{
				echo '<p><strong> Stat file cannot be locked. Please try again later.</p>';
			}
			//Now read the two variables from the file using an fscanf
			fscanf($statfp, "%d %d", $accessno, $queryno);
			$accessno = $accessno + 1;
			$queryno = $queryno + $count;
			fseek($statfp, 0);
			if(false == ftruncate($statfp,0))
			{
				echo '<p><strong> Stat file cannot be truncated. Please try again later.</p>';
			}
			// Now write those variables in the stat File
			fprintf($statfp, "%d %d", $accessno, $queryno);
			if(false == flock($statfp, LOCK_UN))
			{
				echo '<p><strong> Stat file cannot be unlocked. Please try again later.</p>';
			}
			//Now close the file
			fclose($statfp);
		}
